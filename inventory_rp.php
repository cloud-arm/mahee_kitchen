...
To:<input type="text" style="width:223px; padding:4px;" name="d2" id="datepickerd"  value="" autocomplete="off"/>
 ...
 <i class="icon icon-search icon-large"></i> Search
   ...
   $tot=0;
	...
	$result = $db->prepare("SELECT sum(qty), code, name,sum(price), product_id  FROM sales_list WHERE  date BETWEEN '$d1' AND '$d2' GROUP BY product_id ");
	...
	for($i=0; $row = $result->fetch(); $i++){
			...
			?>
                <tr>
                  ...
                  <td><?php 
    $result1 = $db->prepare("SELECT qty  FROM products WHERE  id='$co_id' ");
